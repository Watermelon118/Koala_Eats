...
2026-06-11：正在 `feature/split-role-portals` 拆分四个前端应用，目标是让用户端、商家端、骑手端、管理端物理隔离。
...
- 配置前端本地 `.env.local`。
- 确认认证方案：JWT 或 Cookie。
- 继续细化用户端结算页、模拟支付页、订单状态页。
- 为四个独立前端应用补充真实登录和鉴权边界。
- 为用户端结算和订单状态设计 mock data。
- 把后续用户端页面需要的接口补充到 `docs/API_CONTRACT.md`。
...
当前 feature 目标“四端物理拆分”已完成：四个独立前端应用已创建，用户端不再暴露其他端入口，四端当前 mock 接口已登记。下一步自动检查、提交、合并回 `main` 并删除 feature 分支。
